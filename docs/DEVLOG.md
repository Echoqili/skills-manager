...
## 2026-06-22 回归测试与 UI 细节修复

### 背景
用户反馈管理菜单出现重复图标，同时要求验证 Playwright 可用性并做完整回归测试。Playwright 浏览器下载因锁文件失败，改用 MCP browser 完成全量点击回归。

### 改动清单

#### 1. 管理菜单去除重复图标
- **问题**：`manage-menu-item` 内同时硬编码了 `<span>🔍</span>` 图标与 `<span data-i18n="tab_discover">` 文案，而 i18n 文案本身已包含图标；`applyI18n()` 后每个菜单项出现两个图标。
- **修复**：删除硬编码图标 span，仅保留 `data-i18n` 文案 span。
- **文件**：`web/templates/index.html`

#### 2. 购物车侧边栏标题同步
- **问题**：添加/移除 Skill 后，仅购物车头部的角标和旧版清单页计数更新，侧边栏关闭状态下标题仍显示旧数量 `(0)`。
- **修复**：在 `updateCart()` 中同步更新 `cartSidebarCount` 与 `cartSidebarTitleText`，保证打开侧边栏前数据已是最新。
- **文件**：`web/templates/index.html`

### 验证
- 使用 MCP browser 对 https://skills-manager.onrender.com/ 进行回归测试：
  - 页面加载、统计、分类筛选正常
  - 导航标签（浏览/场景/分类）切换正常
  - 本地/GitHub 搜索、分类过滤正常
  - Skill 卡片加入/移除购物车、角标同步正常
  - 购物车侧边栏打开/关闭/刷新持久化正常
  - 管理菜单各页面可正常打开
  - 导入 Skill 验证并通过
- 测试中发现「查看详情」疑似跳转错误，需后续进一步复现确认。
