...
- 管理 600+ 个来自多个开源项目的 Skills
- 从 GitHub 发现和导入新的 Skills
- 安全审计扫描，识别恶意代码风险
- AI 智能推荐（支持 OpenAI / DeepSeek / 通义千问 等）
- 购物车打包批量下载
- 自动更新流水线
...
Web 界面顶部有 10 个导航标签页，覆盖全部功能。右上角支持**中/英文双语切换**。
...
- **技能卡片**：每张卡片显示 emoji 图标、名称、分类标签、简要描述、文件数/大小统计
- **卡片操作**：
...
- 显示从 GitHub 搜索到的 Skills 仓库
- 每张卡片显示仓库名、Owner、描述、Star 数、语言标识
- 点击可直接跳转到 GitHub 仓库页面

**提示：** 在搜索框输入关键词后，点击 **🌐 全网** 按钮可同时搜索本地 + GitHub。
...
- 按分类展示所有 Skills，每个分类显示 emoji、名称、Skill 数量和进度条
- 支持按分类名称搜索过滤
- 热点分类带"热门"标签
- 点击分类展开该分类下所有 Skills

**可用分类：**
敏捷开发 / AI 安全 / API 设计 / DDD / 设计技能 / 开发质量 / 开发工作流 / GitHub 项目 / 独立黑客 / 产品管理 / QA 测试 / Scrum / Skills 创建 / 通用 Skills / 常用工具 / AI 产品等
...
**我的 Skills：** 列出所有已导入的自定义 Skills，支持删除。
...
> **安全说明：** API Key 仅保存在本地 `data/ai-config.json` 文件中，不会上传到任何第三方。
...
A：使用 **导入 Skill** 选项卡，支持手动创建 Markdown 格式的 Skill，也可以从 GitHub 仓库直接导入。
...
A：将需要的 Skills 逐一点击"加入清单"，然后到 **📦 打包清单** 标签页点击"打包下载选中项"。
